...
def prettify_ast(ast, level = 0, indent = "  "):
    ...
    if type(ast) == list:
        ...
        return (ind + styles["Delimiter"] + "[]" + fg.rs) if len(ast) == 0 else (repr + ind + styles["Delimiter"] + "]" + fg.rs)
    ...
    if type(ast) == dict:
        ...
        for k, v in ast.items():
            ...
            for i, l in enumerate(val_rep):
                if i == 0: l = l.lstrip()
                if i == len(val_rep) - 1: l = l.rstrip()
                repr += l + "\n"

        return (styles["Delimiter"] + "{}" + fg.rs) if len(ast.keys()) == 0 else (repr + ind + styles["Delimiter"] + "}" + fg.rs)
